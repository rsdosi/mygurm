...
// lib/room-protocol.ts
var SHOTS = 4;
var CODE_ALPHABET = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
var CODE_LENGTH = 5;
function randomCode(rand = Math.random) {
  let out = "";
...
var RoomServer = class extends Server {
  ...
  async runSequence() {
    ...
    const alive = /* @__PURE__ */ __name(() => token === this.runToken, "alive");
...
var RESERVE_TTL = 2 * 60 * 1e3;
var ACTIVE_TTL = 12 * 60 * 60 * 1e3;
var CORS = {
  "access-control-allow-origin": "*",
...
var LobbyServer = class extends Server {
  ...
  isTaken(code) {
    ...
    return exp !== void 0 && exp > Date.now();
  }
  async createCode() {
    this.prune();
    let code = randomCode();
  ...
  iceServers() {
    const env3 = this.env;
    const stun = env3.STUN_URLS ? env3.STUN_URLS.split(",").map((s) => s.trim()).filter(Boolean) : DEFAULT_STUN_URLS;
    const servers = [{ urls: stun }];
    if (env3.TURN_URL) {
      const urls = env3.TURN_URL.split(",").map((s) => s.trim()).filter(Boolean);
      servers.push({
        urls,
        username: env3.TURN_USERNAME,
        credential: env3.TURN_CREDENTIAL
      });
    }
    return servers;
  }
  async onRequest(req) {
    ...
    if (req.method === "GET") {
      const body = { iceServers: this.iceServers() };
      return json(body);
    }
    if (req.method === "POST") {
      let payload = {};
...
// .wrangler/tmp/bundle-f7eatU/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-f7eatU/middleware-loader.entry.ts
